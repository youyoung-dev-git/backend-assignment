...
## 과제 개요
주문 시스템의 비즈니스 로직과 REST API를 구현해주세요.

### 구현 요구사항
1. **비즈니스 로직 구현** (필수)
   - 제공된 테스트 코드 1~5번을 모두 통과해야 합니다
   - 6번 동시성 테스트는 선택사항입니다

2. **REST API 구현** (필수)
   - 주문 시스템의 주요 기능을 REST API로 제공해주세요
   - API 설계(엔드포인트, 요청/응답 형식)는 자유입니다
   - `/com/example/order/controller` 패키지에 구현하시면 됩니다

---

## 1. 비즈니스 로직 테스트 (필수)

아래 테스트를 모두 통과하도록 구현해주세요.
...
### 자유 설계 항목
- 엔드포인트 구조 (예: `/api/orders`, `/orders` 등)
- 요청/응답 형식
- API 테스트 코드 작성 여부

### 확인 방법
- Swagger UI에서 확인 가능: `http://localhost:8080/`
